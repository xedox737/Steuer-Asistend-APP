...
class ReceiptDetailComposeTest {
        ...
        ui.setContent {
            MaterialTheme(colorScheme = lightColorScheme()) {
                ReceiptDetailLayout(receipt, "Wohnung 1", if (assigned) listOf(link to transaction) else emptyList(), bitmap,
                    onBack = {}, onNavigate = { destination = it }, onShare = { shares++ }, onFullScreen = {},
    ...
    @Test fun editAndAdditionalDataStayOnOneDetailPage() {
        show()
        ui.onNodeWithText("Bearbeiten").performScrollTo().performClick()
        ui.onNodeWithText("Editor schließen").assertExists().performScrollTo().performClick()

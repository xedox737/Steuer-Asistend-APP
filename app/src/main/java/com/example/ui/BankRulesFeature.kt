...
private fun conditionSummary(r:BankLearningRule)=listOfNotNull(r.counterpartyPattern.takeIf{it.isNotBlank()}?.let{"Gegenpartei enthält „$it“"},r.purposeTerms.takeIf{it.isNotBlank()}?.let{"Zweck: ${it.replace('|',',')}"},r.amountMin?.let{"ab %.2f €".format(it)},r.amountMax?.let{"bis %.2f €".format(it)},r.accountId.takeIf{it.isNotBlank()}?.let{"Konto $it"},r.transactionDirection.takeIf{it!=BankRuleDirection.ANY}).joinToString(" • ").ifBlank{"allgemeine Bankbuchung"}
private fun targetSummary(r:BankLearningRule)=listOfNotNull(r.propertyId.takeIf{it.isNotBlank()}?.let{"Objekt $it"},r.unitId.takeIf{it.isNotBlank()}?.let{"Einheit $it"},r.receiptVendorTarget.takeIf{it.isNotBlank()}?.let{"Händler $it"},r.receiptCategoryTarget.takeIf{it.isNotBlank()}?.let{"Kategorie $it"},r.paymentMethodTarget.takeIf{it.isNotBlank()}?.let{"Zahlungsart $it"}).joinToString(" • ").ifBlank{"passende Belege"}
private fun sourceLabel(v:String)=when(v){BankRuleSource.LEARNED_FROM_CONFIRMATIONS->"Aus Bestätigungen gelernt";BankRuleSource.USER_CREATED->"Vom Nutzer erstellt";BankRuleSource.USER_EDITED->"Vom Nutzer bearbeitet";else->v}

@Composable
private fun RuleEditorDialog(existing:BankLearningRule?,onDismiss:()->Unit,onSave:(BankLearningRule)->Unit) {
    var name by remember(existing){mutableStateOf(existing?.displayName.orEmpty())}; var cp by remember(existing){mutableStateOf(existing?.counterpartyPattern.orEmpty())}; var purpose by remember(existing){mutableStateOf(existing?.purposeTerms.orEmpty())}; var min by remember(existing){mutableStateOf(existing?.amountMin?.toString().orEmpty())}; var max by remember(existing){mutableStateOf(existing?.amountMax?.toString().orEmpty())}; var account by remember(existing){mutableStateOf(existing?.accountId.orEmpty())}; var property by remember(existing){mutableStateOf(existing?.propertyId.orEmpty())}; var unit by remember(existing){mutableStateOf(existing?.unitId.orEmpty())}; var vendor by remember(existing){mutableStateOf(existing?.receiptVendorTarget.orEmpty())}; var category by remember(existing){mutableStateOf(existing?.receiptCategoryTarget.orEmpty())}; var payment by remember(existing){mutableStateOf(existing?.paymentMethodTarget.orEmpty())}; var direction by remember(existing){mutableStateOf(existing?.transactionDirection?:BankRuleDirection.ANY)}
    AlertDialog(onDismissRequest=onDismiss,title={Text(if(existing==null)"Neue Bankregel" else "Bankregel bearbeiten")},text={Column(verticalArrangement=Arrangement.spacedBy(6.dp)){
        OutlinedTextField(name,{name=it},label={Text("Regelname")}); OutlinedTextField(cp,{cp=it},label={Text("Gegenpartei enthält")}); OutlinedTextField(purpose,{purpose=it},label={Text("Verwendungszweck enthält")}); Row(horizontalArrangement=Arrangement.spacedBy(4.dp)){ listOf(BankRuleDirection.ANY to "Alle",BankRuleDirection.EXPENSE to "Ausgabe",BankRuleDirection.INCOME to "Einnahme").forEach{(v,l)->FilterChip(direction==v,{direction=v},{Text(l)})} }; OutlinedTextField(min,{min=it},label={Text("Betrag von")}); OutlinedTextField(max,{max=it},label={Text("Betrag bis")}); OutlinedTextField(account,{account=it},label={Text("Konto (optional)")}); OutlinedTextField(property,{property=it},label={Text("Objekt (optional)")}); OutlinedTextField(unit,{unit=it},label={Text("Einheit (optional)")}); OutlinedTextField(vendor,{vendor=it},label={Text("Händler bevorzugen")}); OutlinedTextField(category,{category=it},label={Text("Kategorie bevorzugen")}); OutlinedTextField(payment,{payment=it},label={Text("Zahlungsart vorschlagen")}); Text("Zusammenfassung: Wenn ${cp.ifBlank{"Buchung"}} passt, dann ${category.ifBlank{vendor.ifBlank{"passende Belege"}}} bevorzugen.", color=SlateGray)
    }},confirmButton={Button(onClick={val now=Instant.now().toString(); val id=existing?.ruleId?:"user-rule-${java.util.UUID.randomUUID()}"; onSave((existing?:BankLearningRule(id,name.ifBlank{"Bankregel"})).copy(displayName=name.ifBlank{"Bankregel"},enabled=existing?.enabled?:true,state=BankRuleState.ACTIVE,ruleType=BankRuleType.COMBINED,transactionDirection=direction,counterpartyPattern=cp,purposeTerms=purpose.replace(',', '|'),amountMin=min.replace(',','.').toDoubleOrNull(),amountMax=max.replace(',','.').toDoubleOrNull(),accountId=account,propertyId=property,unitId=unit,receiptVendorTarget=vendor,receiptCategoryTarget=category,paymentMethodTarget=payment,source=existing?.source?:BankRuleSource.USER_CREATED,createdAt=existing?.createdAt?.ifBlank{now}?:now,updatedAt=now))}){Text("Speichern")}},dismissButton={OutlinedButton(onClick=onDismiss){Text("Abbrechen")}})
}

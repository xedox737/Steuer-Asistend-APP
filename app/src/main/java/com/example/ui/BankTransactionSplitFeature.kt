    ...
    if (showTrigger) {
        if (compactTrigger) {
            BankQuickAction(
                ...
                containerColor = androidx.compose.ui.graphics.Color(0xFFF3E8FF)
            )
        } else if (transaction.reconciliationStatus == BankReconciliationStatus.OPEN || transaction.reconciliationStatus == BankReconciliationStatus.PARTIAL) {

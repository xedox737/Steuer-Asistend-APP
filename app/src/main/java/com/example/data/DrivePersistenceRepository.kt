...
class DrivePersistenceRepository(
        ...
        for (i in 0 until array.length()) {
            ...
            list.add(
                WohneinheitStatus(
                    ...
                    unitId = obj.optString("unitId", StableDocumentIdentity.legacyUnitId(StableDocumentIdentity.LEGACY_PROPERTY_ID, obj.getString("name")))
                )
            )
        }
        return list
    ...
    fun getWohneinheitenFromPrefs(metadata: PropertyMetadata?): List<WohneinheitStatus> {
        ...
        return unitNames.mapIndexed { index, name ->
            WohneinheitStatus(
                ...
                unitId = unitPrefs.getString("unit_id_$name", null) ?: unitPrefs.getString("unit_id_index_$index", null)
                    ?: StableDocumentIdentity.legacyUnitId(propertyId, name).also {
    ...
    suspend fun getReceiptTargetFolderId(
        ...
        val route = DocumentDrivePathResolver.route(
            ...
            type = ManagedDocumentType.RECHNUNG,
    ...
    suspend fun syncReceiptToDrive(
        ...
        try {
            ...
            val targetFolderId = getReceiptTargetFolderId(
                accessToken, config.rootFolderId, currentReceipt, config.systemFolderId, context,
            ...
            var contentSha256 = ""
            var actualDriveFolderId = currentReceipt.driveFolderId
            var actualDriveFilename: String? = null
            ...
            if (!driveFileId.isNullOrBlank()) {
                GoogleDriveClient.getFileMetadata(accessToken, driveFileId!!)?.let {
                    actualDriveFolderId = it.parentIds.singleOrNull() ?: actualDriveFolderId
                    actualDriveFilename = it.name
                }
            }

            if (driveFileId.isNullOrEmpty()) {
                val legacyFile = GoogleDriveClient.findFileByReceiptPropertiesGlobally(accessToken, currentReceipt.internalId, "ORIGINAL")
                if (legacyFile != null) {
            ...
            if (fileBytes != null && fileBytes.isNotEmpty()) {
                ...
                if (needsCreate) {
                    ...
                    if (uploadedId != null) {
                        ...
                        actualDriveFolderId = targetFolderId
    ...
    fun generateStandardizedReceiptFilename(receipt: Receipt, extension: String): String {
        return DocumentFilenameGenerator.receipt(receipt, extension)
    }
    ...
    suspend fun repairAndUploadOriginalDocument(
        ...
        try {
            ...
            val targetFolderId = getReceiptTargetFolderId(
                accessToken, config.rootFolderId, receipt, config.systemFolderId, context,
                localRepository.getPropertyMetadata()
            )
    ...
    private suspend fun ensureDocumentFolder(
        accessToken: String,
        config: DriveAppConfig,
    ...
    suspend fun ensurePropertyDocumentStructure(accessToken: String, config: DriveAppConfig): Boolean {
        val property = localRepository.getPropertyMetadata() ?: PropertyMetadata()
        val rootName = DocumentDrivePathResolver.propertyFolderName(property.name, property.adresse)
        val root = GoogleDriveClient.getOrCreateFolder(
            accessToken, rootName, config.rootFolderId, "documents/${property.propertyId}/root", config.systemFolderId, context
        ) ?: return false
        val paths = listOf(
            listOf("00_Stammdaten", "01_Kauf_Eigentum"), listOf("00_Stammdaten", "02_Grundstueck_Gebaeude"),
            listOf("00_Stammdaten", "03_Energie_Technik"), listOf("00_Stammdaten", "04_Versicherungen"),
            listOf("00_Stammdaten", "05_Steuer_Grundlagen"), listOf("00_Stammdaten", "06_Sonstige_Objektunterlagen"),
            listOf("01_Einheiten"), listOf("02_Belege"), listOf("03_Finanzierung_AfA", "Darlehen"),
            listOf("03_Finanzierung_AfA", "Zinsunterlagen"), listOf("03_Finanzierung_AfA", "Kaufpreisaufteilung"),
            listOf("03_Finanzierung_AfA", "AfA"), listOf("04_Sanierungen"), listOf("05_Steuerberater_Exporte"), listOf("_System")
        )
        for (path in paths) {
            var parent = root
            path.forEachIndexed { index, segment ->
                val canonical = "documents/${property.propertyId}/${path.take(index + 1).joinToString("/") { normalizePathSegment(it) }}"
                parent = GoogleDriveClient.getOrCreateFolder(accessToken, segment, parent, canonical, config.systemFolderId, context) ?: return false
            }
        }
        return true
    }

    /** Uploads a non-receipt document exactly once. Existing originals are only verified, never overwritten. */
    suspend fun syncManagedDocumentToDrive(
        accessToken: String,
        config: DriveAppConfig,
        documentId: String
    ): Boolean {
        ...
        if (existingByHash != null) {
            localRepository.upsertManagedDocument(document.copy(
                driveFileId = existingByHash.driveFileId,
                driveFolderId = existingByHash.driveFolderId,
                sha256 = hash,
                migrationStatus = "IDENTISCHES_ORIGINAL_VERKNUEPFT",
        ...
        val fileId = GoogleDriveClient.uploadFile(
            ...
            appProperties = mapOf(
                ...
                "documentId" to document.documentId, "propertyId" to document.propertyId,
    ...
    suspend fun updateManagedDocumentIndex(accessToken: String, config: DriveAppConfig): Boolean {
        val documents = localRepository.getAllManagedDocuments()
        val json = JSONObject().apply {
            put("schemaVersion", 1)
            put("documents", JSONArray().apply {
                documents.forEach { document -> put(JSONObject().apply {
                    put("documentId", document.documentId); put("propertyId", document.propertyId)
                    put("unitId", document.unitId ?: JSONObject.NULL); put("receiptInternalId", document.receiptInternalId ?: JSONObject.NULL)
                    put("documentType", document.documentType); put("documentDate", document.documentDate)
                    put("storedFilename", document.storedFilename); put("driveFileId", document.driveFileId ?: JSONObject.NULL)
                    put("driveFolderId", document.driveFolderId ?: JSONObject.NULL); put("sha256", document.sha256)
                    put("fileSizeBytes", document.fileSizeBytes); put("reviewStatus", document.reviewStatus); put("updatedAt", document.updatedAt)
                }) }
            })
        }
    ...
    private fun localRepositoryUnitLabel(unitId: String?): String? {
        if (unitId.isNullOrBlank()) return null
        val prefs = context.getSharedPreferences("wohneinheiten_prefs", Context.MODE_PRIVATE)
        return prefs.all.keys.firstOrNull { it.startsWith("unit_id_") && prefs.getString(it, null) == unitId }
    ...
    suspend fun previewReceiptDocumentMigration(accessToken: String, config: DriveAppConfig): DocumentMigrationPreview {
        ...
        val items = localReceipts.mapNotNull { receipt ->
            ...
            val route = DocumentDrivePathResolver.route(property.propertyId, property.name, property.adresse, ManagedDocumentType.RECHNUNG, receipt.datum)
            val ext = meta?.name?.substringAfterLast('.', receipt.storedFilename?.substringAfterLast('.', "bin") ?: "bin") ?: "bin"
            ...
            DocumentMigrationCandidate(
                ...
                driveFileId = fileId,
                currentFolderId = meta?.parentIds?.singleOrNull().orEmpty(),
                currentFilename = meta?.name ?: receipt.storedFilename.orEmpty(),
    ...
    suspend fun executeReceiptDocumentMigration(
        accessToken: String,
        config: DriveAppConfig,
        preview: DocumentMigrationPreview
    ): DocumentMigrationPreview {
        val property = localRepository.getPropertyMetadata() ?: PropertyMetadata()
        val results = preview.items.map { planned ->
            val candidate = planned.candidate
            if (planned.action == DocumentMigrationAction.PRUEFEN) {
                localRepository.getManagedDocument(StableDocumentIdentity.receiptDocumentId(candidate.receiptInternalId))?.let {
                    localRepository.upsertManagedDocument(it.copy(migrationStatus = DocumentReviewStatus.MIGRATION_PRUEFEN.name, updatedAt = java.time.Instant.now().toString()))
                }
                return@map planned
            }
            val receipt = localRepository.getReceiptByInternalId(candidate.receiptInternalId)
                ?: return@map planned.copy(action = DocumentMigrationAction.PRUEFEN)
            val meta = GoogleDriveClient.getFileMetadata(accessToken, candidate.driveFileId)
                ?: return@map planned.copy(action = DocumentMigrationAction.PRUEFEN)
            val targetFolder = ensureDocumentFolder(accessToken, config, property, ManagedDocumentType.RECHNUNG, receipt.datum)
                ?: return@map planned.copy(action = DocumentMigrationAction.PRUEFEN)
            val bytesBefore = GoogleDriveClient.downloadFileBytes(accessToken, candidate.driveFileId)
                ?: return@map planned.copy(action = DocumentMigrationAction.PRUEFEN)
            val concrete = candidate.copy(currentFolderId = meta.parentIds.singleOrNull().orEmpty(), currentFilename = meta.name, targetFolderId = targetFolder, beforeSha256 = getSha256(bytesBefore))
            val concretePlan = DocumentStorageMigrationPlanner.preview(listOf(concrete)).items.single()
            val journal = DocumentStorageMigrationPlanner.journal(concretePlan)
            localRepository.upsertDocumentMigrationJournal(journal)
            if (concretePlan.action != DocumentMigrationAction.UNVERAENDERT && !GoogleDriveClient.moveAndRenameFile(accessToken, concrete.driveFileId, targetFolder, meta.parentIds, concrete.targetFilename)) {
                localRepository.upsertDocumentMigrationJournal(journal.copy(state = "FAILED", lastError = "Drive-Datei konnte nicht verschoben werden", updatedAt = java.time.Instant.now().toString()))
                return@map concretePlan.copy(action = DocumentMigrationAction.PRUEFEN)
            }
            val after = GoogleDriveClient.downloadFileBytes(accessToken, concrete.driveFileId)
            if (after == null || !DocumentMigrationVerifier.isContentPreserved(concrete.beforeSha256, getSha256(after))) {
                localRepository.upsertDocumentMigrationJournal(journal.copy(state = "VERIFY_FAILED", lastError = "Hashprüfung fehlgeschlagen", updatedAt = java.time.Instant.now().toString()))
                return@map concretePlan.copy(action = DocumentMigrationAction.PRUEFEN)
            }
            val migratedReceipt = receipt.copy(driveFolderId = targetFolder, storedFilename = concrete.targetFilename)
            localRepository.insert(migratedReceipt)
            localRepository.getDocumentsForReceipt(receipt.internalId)
                .filter { it.driveFileId == concrete.driveFileId }
                .forEach { reference ->
                    localRepository.insertDocument(reference.copy(driveFolderId = targetFolder, filename = concrete.targetFilename))
                }
            if (!uploadReceiptMetadata(accessToken, config, migratedReceipt)) {
                localRepository.upsertDocumentMigrationJournal(journal.copy(state = "METADATA_REVIEW", lastError = "Belegmetadaten konnten nicht aktualisiert werden", updatedAt = java.time.Instant.now().toString()))
                return@map concretePlan.copy(action = DocumentMigrationAction.PRUEFEN)
            }
            localRepository.getManagedDocument(StableDocumentIdentity.receiptDocumentId(receipt.internalId))?.let { document ->
                localRepository.upsertManagedDocument(document.copy(
                    driveFolderId = targetFolder,
                    storedFilename = concrete.targetFilename,
                    documentCategory = "02_Belege/${receipt.datum.take(4)}",
                    migrationStatus = "COMPLETED",
                    legacyDriveFolderId = concrete.currentFolderId.takeIf { it != targetFolder },
                    updatedAt = java.time.Instant.now().toString()
                ), receipt)
            }
    ...
    suspend fun resumeInterruptedDocumentMigration(accessToken: String, config: DriveAppConfig): DocumentMigrationPreview {
        val pending = DocumentStorageMigrationPlanner.resume(localRepository.getDocumentMigrationJournal())
        return executeReceiptDocumentMigration(accessToken, config, pending)
    }
}

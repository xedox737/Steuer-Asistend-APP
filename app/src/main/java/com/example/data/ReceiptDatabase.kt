...
val MIGRATION_18_19 = object : androidx.room.migration.Migration(18, 19) {
    override fun migrate(db: androidx.sqlite.db.SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE logbook_trips ADD COLUMN manualKm REAL")
        db.execSQL("ALTER TABLE logbook_trips ADD COLUMN routeProvider TEXT NOT NULL DEFAULT ''")
        db.execSQL("ALTER TABLE logbook_trips ADD COLUMN routeCalculatedAt TEXT NOT NULL DEFAULT ''")
        db.execSQL("ALTER TABLE logbook_trips ADD COLUMN routeDurationSeconds INTEGER")
        db.execSQL("ALTER TABLE logbook_trips ADD COLUMN correctionReason TEXT NOT NULL DEFAULT ''")
        db.execSQL("ALTER TABLE logbook_trips ADD COLUMN correctionNote TEXT NOT NULL DEFAULT ''")
        db.execSQL("ALTER TABLE logbook_trips ADD COLUMN routeSignature TEXT NOT NULL DEFAULT ''")
        db.execSQL("ALTER TABLE standard_routes ADD COLUMN routeSignature TEXT NOT NULL DEFAULT ''")
        db.execSQL("ALTER TABLE standard_routes ADD COLUMN sourceProvider TEXT NOT NULL DEFAULT ''")
        db.execSQL("CREATE INDEX IF NOT EXISTS index_standard_routes_routeSignature ON standard_routes(routeSignature)")
    }
}
...
val MIGRATION_24_25 = object : androidx.room.migration.Migration(24, 25) {
    override fun migrate(db: androidx.sqlite.db.SupportSQLiteDatabase) {
        db.execSQL("""CREATE TABLE IF NOT EXISTS bank_learning_rules (ruleId TEXT NOT NULL PRIMARY KEY, displayName TEXT NOT NULL, enabled INTEGER NOT NULL, state TEXT NOT NULL, ruleType TEXT NOT NULL, transactionDirection TEXT NOT NULL, counterpartyPattern TEXT NOT NULL, counterpartyIbanPattern TEXT NOT NULL, purposeTerms TEXT NOT NULL, amountMin REAL, amountMax REAL, currency TEXT NOT NULL, accountId TEXT NOT NULL, propertyId TEXT NOT NULL, unitId TEXT NOT NULL, receiptVendorTarget TEXT NOT NULL, receiptCategoryTarget TEXT NOT NULL, receiptSubcategoryTarget TEXT NOT NULL, paymentMethodTarget TEXT NOT NULL, evidenceCount INTEGER NOT NULL, successCount INTEGER NOT NULL, rejectionCount INTEGER NOT NULL, confidence INTEGER NOT NULL, source TEXT NOT NULL, createdAt TEXT NOT NULL, updatedAt TEXT NOT NULL, lastMatchedAt TEXT NOT NULL)""")
        db.execSQL("CREATE INDEX IF NOT EXISTS index_bank_learning_rules_enabled ON bank_learning_rules(enabled)")
        db.execSQL("CREATE INDEX IF NOT EXISTS index_bank_learning_rules_accountId ON bank_learning_rules(accountId)")
        db.execSQL("CREATE INDEX IF NOT EXISTS index_bank_learning_rules_propertyId ON bank_learning_rules(propertyId)")
        db.execSQL("CREATE INDEX IF NOT EXISTS index_bank_learning_rules_unitId ON bank_learning_rules(unitId)")
        db.execSQL("""CREATE TABLE IF NOT EXISTS bank_rule_evidence (evidenceId TEXT NOT NULL PRIMARY KEY, candidateKey TEXT NOT NULL, ruleId TEXT NOT NULL, transactionId TEXT NOT NULL, receiptId INTEGER NOT NULL, confirmed INTEGER NOT NULL, counterparty TEXT NOT NULL, direction TEXT NOT NULL, purposeTerms TEXT NOT NULL, accountId TEXT NOT NULL, propertyId TEXT NOT NULL, unitId TEXT NOT NULL, vendorTarget TEXT NOT NULL, categoryTarget TEXT NOT NULL, subcategoryTarget TEXT NOT NULL, paymentMethodTarget TEXT NOT NULL, createdAt TEXT NOT NULL)""")
        db.execSQL("CREATE INDEX IF NOT EXISTS index_bank_rule_evidence_candidateKey ON bank_rule_evidence(candidateKey)")
        db.execSQL("CREATE INDEX IF NOT EXISTS index_bank_rule_evidence_transactionId ON bank_rule_evidence(transactionId)")
        db.execSQL("CREATE INDEX IF NOT EXISTS index_bank_rule_evidence_ruleId ON bank_rule_evidence(ruleId)")
    }
}
...
val MIGRATION_26_27 = object : androidx.room.migration.Migration(26, 27) {
    override fun migrate(db: androidx.sqlite.db.SupportSQLiteDatabase) {
        db.execSQL("CREATE TABLE IF NOT EXISTS bank_loan_assignments (assignmentId TEXT NOT NULL PRIMARY KEY, transactionId TEXT NOT NULL, loanId INTEGER NOT NULL, propertyId TEXT NOT NULL, allocatedAmount REAL NOT NULL, paymentType TEXT NOT NULL, period TEXT NOT NULL, status TEXT NOT NULL, source TEXT NOT NULL, splitStatus TEXT NOT NULL, proposedInterest REAL, proposedPrincipal REAL, interestBasis TEXT NOT NULL, createdAt TEXT NOT NULL, updatedAt TEXT NOT NULL)")
        db.execSQL("CREATE UNIQUE INDEX IF NOT EXISTS index_bank_loan_assignments_transactionId ON bank_loan_assignments(transactionId)")
        db.execSQL("CREATE INDEX IF NOT EXISTS index_bank_loan_assignments_loan_period ON bank_loan_assignments(loanId, period)")
        db.execSQL("CREATE INDEX IF NOT EXISTS index_bank_loan_assignments_propertyId ON bank_loan_assignments(propertyId)")
        db.execSQL("CREATE TABLE IF NOT EXISTS bank_recurring_patterns (patternId TEXT NOT NULL PRIMARY KEY, enabled INTEGER NOT NULL, direction TEXT NOT NULL, normalizedCounterparty TEXT NOT NULL, purposeFingerprint TEXT NOT NULL, typicalAmount REAL NOT NULL, amountTolerance REAL NOT NULL, cadence TEXT NOT NULL, typicalDay INTEGER NOT NULL, accountId TEXT NOT NULL, propertyId TEXT NOT NULL, occurrenceCount INTEGER NOT NULL, confidence INTEGER NOT NULL, lastOccurrence TEXT NOT NULL, nextExpectedStart TEXT NOT NULL, nextExpectedEnd TEXT NOT NULL, reasonsText TEXT NOT NULL, createdAt TEXT NOT NULL, updatedAt TEXT NOT NULL)")
        db.execSQL("CREATE INDEX IF NOT EXISTS index_bank_recurring_patterns_accountId ON bank_recurring_patterns(accountId)")
        db.execSQL("CREATE INDEX IF NOT EXISTS index_bank_recurring_patterns_propertyId ON bank_recurring_patterns(propertyId)")
        db.execSQL("CREATE INDEX IF NOT EXISTS index_bank_recurring_patterns_enabled ON bank_recurring_patterns(enabled)")
    }
}
...
val MIGRATION_29_30 = object : androidx.room.migration.Migration(29, 30) {
    override fun migrate(db: androidx.sqlite.db.SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE bank_transactions ADD COLUMN category TEXT NOT NULL DEFAULT ''")
        db.execSQL("ALTER TABLE bank_transactions ADD COLUMN subcategory TEXT NOT NULL DEFAULT ''")
    }
}

val MIGRATION_30_31 = object : androidx.room.migration.Migration(30, 31) {
    override fun migrate(db: androidx.sqlite.db.SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE property_metadata ADD COLUMN bildPfad TEXT NOT NULL DEFAULT ''")
        db.execSQL("ALTER TABLE property_metadata ADD COLUMN objektart TEXT NOT NULL DEFAULT 'Mehrfamilienhaus'")
        db.execSQL("ALTER TABLE property_metadata ADD COLUMN status TEXT NOT NULL DEFAULT 'Aktiv'")
        db.execSQL("ALTER TABLE property_metadata ADD COLUMN notizen TEXT NOT NULL DEFAULT ''")
    }
}
...
abstract class AppDatabase : RoomDatabase() {
    ...
    abstract fun bankLoanAssignmentDao(): BankLoanAssignmentDao
    abstract fun bankRecurringPatternDao(): BankRecurringPatternDao

...
# Matching must never suggest receipts for private/transfer movements.
text = text.replace(
...
text = path.read_text()
# Keep replacements conservative: whenever missing/open checks are based only on reconciliationStatus,
# prepend the central policy gate where the known patterns occur.
text = text.replace(
